                    ...
                    <c:if test="${not empty param.error}">
                        <div class="alert alert-error">
                            <c:choose>
                                ...
                                <c:when test="${param.error == 'missingDescription'}">Vui lòng nhập mô tả chi tiết.
                                </c:when>
                                <c:when test="${param.error == 'invalidCategory'}">Danh mục không hợp lệ. Vui lòng chọn
                                    lại.</c:when>
                                <c:when test="${param.error == 'invalidPriority'}">Mức ưu tiên không hợp lệ.</c:when>
                                <c:when test="${param.error == 'invalidTitleFormat'}">Tiêu đề chỉ được chứa chữ cái và
                                    khoảng trắng (không số, không ký tự đặc biệt).</c:when>
                                <c:when test="${param.error == 'invalidDescriptionFormat'}">Mô tả chỉ được chứa chữ cái
                                    và khoảng trắng (không số, không ký tự đặc biệt).</c:when>
                                <c:when test="${param.error == 'invalidTextFormat'}">Tiêu đề/Mô tả không hợp lệ: chỉ cho
                                    phép chữ cái và khoảng trắng.</c:when>
                                <c:otherwise>Dữ liệu không hợp lệ. Vui lòng kiểm tra lại thông tin nhập.</c:otherwise>
                            </c:choose>
                        </div>
                    </c:if>
                    <form
                        action="${pageContext.request.contextPath}/incident?action=${not empty incident ? 'update' : 'insert'}"
                        method="post">
                                ...
                                <div class="info-grid">
                                    ...
                                    <div class="info-item">
                                        ...
                                        <div class="info-value">
                                            <span
                                                class="status-badge status-${incident.status}">${incident.status}</span>
                                        </div>
                                    </div>
                                    <div class="info-item">
                            ...
                            <div class="form-row">
                                <div class="form-group">
                                    ...
                                    <input type="text" id="title" name="title" class="form-control"
                                        value="${incident.title}" placeholder="Mô tả ngắn gọn sự cố..." required>
                                </div>
                                <div class="form-group">
                                    <label for="priority">
                                        Mức ưu tiên
                                    ...
                                    <select id="priority" name="priority" class="form-control">
                                        <option value="LOW" ${incident.priority=='LOW' ? 'selected' : '' }>🟢 Thấp
                                        </option>
                                        <option value="MEDIUM" ${incident.priority=='MEDIUM' ? 'selected' : '' }>🟡
                                            Trung bình</option>
                                        <option value="HIGH" ${incident.priority=='HIGH' ? 'selected' : '' }>🟠 Cao
                                        </option>
                                    </select>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <c:choose>
                                            <c:when test="${isEndUserEditingOwnTicket}">
                                                <input type="text" class="form-control" value="${incident.status}"
                                                    readonly>
                                            ...
                                            <c:otherwise>
                                                <select id="status" name="status" class="form-control">
                                                    <option value="NEW" ${incident.status=='NEW' ? 'selected' : '' }>🆕
                                                        Mới</option>
                                                    <option value="IN_PROGRESS" ${incident.status=='IN_PROGRESS'
                                                        ? 'selected' : '' }>🔄 Đang xử lý</option>
                                                    <option value="RESOLVED" ${incident.status=='RESOLVED' ? 'selected'
                                                        : '' }>✅ Đã xử lý</option>
                                                    <option value="CANCELLED" ${incident.status=='CANCELLED'
                                                        ? 'selected' : '' }>❌ Đã hủy</option>
                                                </select>
                                                <div class="help-text">Cập nhật trạng thái trong quá trình xử lý sự cố
                                                </div>
                                            </c:otherwise>
                                        </c:choose>
                                    </div>
                                ...
                                <c:choose>
                                    <c:when test="${sessionScope.user.roleId == 1}">
                                        <div class="suggest-box">
                                            ...
                                            <div class="help-text">
                                                Dựa trên tiêu đề/mô tả bạn nhập, hệ thống sẽ gợi ý các sự cố có thể
                                                trùng.
                                            ...
                                            <div class="suggest-list" id="suggestList">
                                                <div class="help-text" id="suggestHint">Nhập tiêu đề/mô tả để xem gợi ý.
                                                </div>
                                            </div>
                                        </div>
                                    </c:when>
                                    <c:otherwise>
                                        <div class="suggest-box">
                                            ...
                                            <div class="help-text">
                                                Agent/Expert: hệ thống sẽ gợi ý theo <b>ticket number</b>, <b>mã lỗi</b>
                                                (trong tiêu đề/mô tả/cause/solution) và
