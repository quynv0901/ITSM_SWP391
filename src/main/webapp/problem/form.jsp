...
<div class="container-fluid bg-white p-4 rounded shadow-sm" style="max-width: 800px; margin: auto;">
    <h2 class="h4 text-primary mb-4 border-bottom pb-2">${not empty problem ? 'Update Problem Ticket' : 'Create Problem Ticket'}</h2>
    ...
    <form action="${pageContext.request.contextPath}/problem?action=${not empty problem ? 'update' : 'insert'}" method="post">
        ...
        <div class="mb-3">
            <label for="title" class="form-label fw-bold">Problem Title / Summary <span class="text-danger">*</span></label>
        ...
        <c:if test="${not empty problem}">
            <div class="mb-3">
                ...
                <c:if test="${empty problem.assignedTo}">
                    <div class="form-text text-warning"><i class="bi bi-info-circle"></i> Status cannot be changed to In Progress/Resolved until this problem is assigned.</div>
                </c:if>
            </div>
        </c:if>
        ...
        <div class="d-grid gap-2 mt-4">
            ...
            <a href="${pageContext.request.contextPath}/problem?action=list" class="btn btn-outline-secondary">Cancel and Return</a>
        </div>
    </form>
</div>
