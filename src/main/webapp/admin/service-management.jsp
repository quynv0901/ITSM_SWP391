...
<div class="container-fluid bg-white p-4 rounded shadow-sm">
    <div class="d-flex justify-content-between align-items-center mb-4 flex-wrap gap-2">
        ...
        <div class="d-flex gap-2 flex-wrap">
            <button type="button" class="btn btn-secondary" onclick="submitBulkAction('INACTIVE')"><i class="bi bi-pause-circle"></i> Ngừng kích hoạt hàng loạt</button>
            <button type="button" class="btn btn-success" onclick="submitBulkAction('ACTIVE')"><i class="bi bi-play-circle"></i> Kích hoạt hàng loạt</button>
            <button type="button" class="btn btn-primary" data-bs-toggle="modal" data-bs-target="#createServiceModal"><i class="bi bi-plus-circle"></i> Tạo dịch vụ mới</button>
        </div>
    </div>
        ...
        <div class="table-responsive">
            <table class="table table-hover table-bordered align-middle mt-3">
                <thead class="table-light">
                    <tr>
                        <th style="width:40px;" class="text-center"><input type="checkbox" id="selectAll" class="form-check-input" onclick="toggleAll(this)"></th>
                        <th>Mã dịch vụ</th>
                        <th>Tên dịch vụ</th>
                        <th>Mô tả</th>
                ...
                <tbody>
                    <c:forEach var="svc" items="${services}">
                        <tr>
                            <td class="text-center"><input type="checkbox" name="serviceIds" value="${svc.serviceId}" class="rowCheckbox form-check-input"></td>
                            ...
                            <td class="text-center">
                                <div class="d-flex justify-content-center gap-1 flex-wrap">
                                    <a href="${pageContext.request.contextPath}/admin-services?action=detail&id=${svc.serviceId}&q=${keyword}&status=${status}&page=${currentPage}" class="btn btn-info btn-sm text-white"><i class="bi bi-eye"></i></a>
                                    <a href="${pageContext.request.contextPath}/admin-services?action=edit&id=${svc.serviceId}&q=${keyword}&status=${status}&page=${currentPage}" class="btn btn-warning btn-sm text-white"><i class="bi bi-pencil"></i></a>
                                    <button type="button" class="btn btn-${svc.status == 'ACTIVE' ? 'secondary' : 'success'} btn-sm" onclick="toggleOne('${svc.serviceId}')"><i class="bi ${svc.status == 'ACTIVE' ? 'bi-pause-circle' : 'bi-play-circle'}"></i></button>
                                    <button type="button" class="btn btn-danger btn-sm" onclick="confirmDeleteOne('${svc.serviceId}')"><i class="bi bi-trash"></i></button>
                                </div>
                            </td>
                        </tr>
                    </c:forEach>
        ...
        <nav>
            <ul class="pagination mb-0">
                <li class="page-item ${currentPage <= 1 ? 'disabled' : ''}">
                    <a class="page-link" href="${pageContext.request.contextPath}/admin-services?page=${currentPage - 1}&q=${keyword}&status=${status}">Trước</a>
                </li>
                <c:forEach begin="1" end="${totalPages}" var="p">
                    <li class="page-item ${p == currentPage ? 'active' : ''}">
                        <a class="page-link" href="${pageContext.request.contextPath}/admin-services?page=${p}&q=${keyword}&status=${status}">${p}</a>
                    </li>
                </c:forEach>
                <li class="page-item ${currentPage >= totalPages ? 'disabled' : ''}">
                    <a class="page-link" href="${pageContext.request.contextPath}/admin-services?page=${currentPage + 1}&q=${keyword}&status=${status}">Sau</a>
                </li>
            </ul>
        </nav>
    </div>
...
<script>
function toggleAll(source){document.querySelectorAll('.rowCheckbox').forEach(cb=>cb.checked=source.checked);}
...
function validateServiceFields(prefix){
    ...
    if (!serviceName){ setErr(prefix + 'ServiceNameError', 'Tên dịch vụ không được để trống.'); valid = false; }
    if (!serviceCode){ setErr(prefix + 'ServiceCodeError', 'Mã dịch vụ không được để trống.'); valid = false; }
    else if (!/^[A-Za-z0-9._-]+$/.test(serviceCode)){ setErr(prefix + 'ServiceCodeError', 'Mã dịch vụ chỉ được chứa chữ, số, dấu gạch ngang, gạch dưới và dấu chấm.'); valid = false; }
    if (!estimatedDeliveryDay){ setErr(prefix + 'EstimatedDeliveryDayError', 'Số ngày dự kiến không được để trống.'); valid = false; }
    else if (!/^\d+$/.test(estimatedDeliveryDay)){ setErr(prefix + 'EstimatedDeliveryDayError', 'Số ngày dự kiến phải là số nguyên không âm.'); valid = false; }
    return valid;
}
function validateCreateServiceForm(){ return validateServiceFields('create'); }
function validateEditServiceForm(){ return validateServiceFields('edit'); }
function submitBulkAction(actionType){
    const checkboxes = document.querySelectorAll('.rowCheckbox:checked');
    if (checkboxes.length === 0){ alert('Vui lòng chọn ít nhất một dịch vụ.'); return; }
...
function toggleOne(id){
    if(confirm('Bạn có muốn thay đổi trạng thái của dịch vụ này không?')){
        ...
        form.method='POST'; form.action='${pageContext.request.contextPath}/admin-services';
...
document.addEventListener('DOMContentLoaded', function(){
    ...
    if (openModal === 'detail') new bootstrap.Modal(document.getElementById('detailServiceModal')).show();
    if (openModal === 'create') new bootstrap.Modal(document.getElementById('createServiceModal')).show();
    if (openModal === 'edit') new bootstrap.Modal(document.getElementById('editServiceModal')).show();
});
</script>
